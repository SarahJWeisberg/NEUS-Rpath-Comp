...
##4.3 Comparing nonselective/plausible
...
```
##4.4 Figure 4: Flow metric comparisons (plausible to nonselective)
```{r}
games_test <- ASC_CAP %>% filter(condition == "plausible") %>% games_howell_test(ASC_CAP ~ EPU, detailed = T)

Figure_4b<-ASC_CAP %>% filter(condition == "plausible") %>%
  ...
  labs(y="Relative Ascendency",x="",title = "Plausible Condition")+
  theme(legend.position = "none",text = element_text(size=12))

games_test <- ASC_CAP %>% filter(condition == "nonselective") %>% games_howell_test(ASC_CAP ~ EPU, detailed = T)

Figure_4a<-ASC_CAP %>% filter(condition == "nonselective") %>%
  ggplot(aes(y=ASC_CAP,x=EPU,color=EPU))+
  geom_hline(yintercept = 0.4,color="black",linetype="dashed",linewidth=1)+
  geom_quasirandom(size = 1, width = .2, alpha = .25)+
  stat_summary(fun = mean, geom = "point", shape = 95,size = 24) + 
  ...
  theme_Publication()+
  stat_pvalue_manual(games_test,label = "p.adj.signif",y.position = 0.5,step.increase = 0.05)+
  labs(y="Relative Ascendency",x="",title="Nonselective Condition")+
  theme(legend.position = "none",text = element_text(size=12))

Figure_4<-Figure_4a+Figure_4b
Figure_4<-Figure_4+plot_annotation(tag_levels = "A")
Figure_4
ggsave(filename = "Fig5.jpg",Figure_4,width=9,height = 8.5,dpi = 300)
```
##4.5 Figure 5: Group-level flow distributions
```{r}
Figure_5<-flow %>%
  filter(EPU == "GOM")%>%
  filter(condition == "nonselective") %>%
  ggplot(aes(x=flow))+
...
ggsave(filename = "Fig6.jpg",Figure_5,width=13.65,height = 10.8,dpi = 300)

```

##5.6 Figures 6 and 7: Flow metric comparisons (plausible to nonselective)
...
flow <- flow %>% mutate(EPU_condition = paste(EPU,condition,sep = " ")) %>%
    mutate(EPU_condition = fct_relevel(EPU_condition,"GOM nonselective","GOM plausible","GB nonselective","GB plausible","MAB nonselective","MAB plausible")) %>% add_significance("p")
...
Figure_7
ggsave(filename = "Fig7.jpg",Figure_6,width=13.65,height = 10.8,dpi = 300)
ggsave(filename = "Fig8.jpg",Figure_7,width=13.65,height = 10.8,dpi = 300)
