...
Figure_4a<-ASC_CAP %>% filter(condition == "initial") %>%
  ...
  theme_Publication()+
  stat_pvalue_manual(games_test,label = "p.adj.signif",y.position = 0.5,step.increase = 0.05)+
  labs(y="A/DC",x="",title="Initial Condition")+
  theme(legend.position = "none",text = element_text(size=12))
...
flow %>%
  ...
  ggplot(aes(x=flow))+
  geom_density(alpha = 0.4, fill= my.pal2[4])+
...
flow <- flow %>% mutate(EPU_condition = paste(EPU,condition,sep = " ")) %>%
    mutate(EPU_condition = fct_relevel(EPU_condition,"GOM initial","GOM constrained","GB initial","GB constrained","MAB initial","MAB constrained"))
...
Figure_6c<-flow %>%
  ...
  theme(legend.position = "none")+
  coord_cartesian(expand = F)+
  theme(text = element_text(size=12),legend.position = "bottom")#,legend.key.spacing.x = unit(5, 'lines'))+
...
Figure_7c<-flow %>%
  ...
  #,legend.text = element_text(margin = margin(r = 30, ,l=5,unit = "pt")))
