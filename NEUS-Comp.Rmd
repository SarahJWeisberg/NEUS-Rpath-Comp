...
#stats tools
library(stats);library(forcats); 
```
Load in the balanced models
```{r}
...
#GOM
# GOM<-cbind(nrow(GOM.params$model[Type <  2, ]),nrow(GOM.params$model[Type == 3, ]))
# #GB
# GB<-cbind(nrow(GB.params.bal$model[Type <  2, ]),1,nrow(GB.params.bal$model[Type == 3, ]))
# #MAB
# MAB<-cbind(nrow(MAB.rpath.params$model[Type <  2, ]),nrow(MAB.rpath.params$model[Type == 3, ]))
