...
#3 Analyze and compare ecosense outputs Load ecosense outputs - nonselective and plausible
...
GB_flow<- GB_flow %>% mutate(EPU = "GB") %>% mutate(flow=as.numeric(flow)) %>% 
  mutate(condition = c(rep("plausible",length(GB_sense_bound_bac)*(nliving+ndead)),rep("nonselective",length(GB_sense_unbound_bac)*(nliving+ndead))))
...
Figure_3
```
##5.4 Figure 4: Flow metric comparisons (plausible to nonselective)
```{r}
