...
#0 Prep Load packages and models for analysis
...
```{r}
#GB
url<-"https://github.com/NOAA-EDAB/GBRpath/blob/max_clam_edits/data/GB.bal.rda?raw=true"
download.file(url,destfile = "GB.bal.rda")
load("GB.bal.rda")

url<-"https://github.com/NOAA-EDAB/GBRpath/blob/max_clam_edits/data/GB.params.bal.rda?raw=true"
download.file(url,destfile = "GB.params.bal.rda")
load("GB.params.bal.rda")
rm(url)
...
#1 Comparing the balanced models ##1.1 Groups & Fleets
...
#fix naming mismatches (temp fix)
all_groups <- all_groups %>% mutate(group = ifelse(group == "Seals","Pinnipeds",
                                                   ifelse(group=="BalWhale","BaleenWhales",
                                                          ifelse(group=="Seabirds","SeaBirds",
                                                                 ifelse(group=="ToothWhale","Odontocetes",group)))))


#pbs
all_groups<-all_groups %>%
  mutate(PB = c(GOM.params$model$PB,GB.params.bal$model$PB,MAB.rpath.params$model$PB), 
         QB = c(GOM.params$model$QB,GB.params.bal$model$QB,MAB.rpath.params$model$QB),
         Biomass = c(GOM.params$model$Biomass,GB.params.bal$model$Biomass,MAB.rpath.params$model$Biomass),
...
all_groups %>% filter(!group == "Phytoplankton")%>%  
...
ggplot(data=all_groups,aes(x=model,y=Biomass,color=model)) +
  geom_bar(stat="identity")+
...
#2 Analyze and compare ecosense outputs Load ecosense outputs - bounded and unbounded
...
#3 ENA on Ecosense outputs ##3.1 Network conversion of MAB ecosense outputs
...
##3.2 Network conversion of GOM ecosense outputs
...
ASC_CAP<-rbind(ASC_CAP_GOM,ASC_CAP_MAB)
```

#4 Figures \## 4.1 Figure 1: Map
...
NEUS <- ggplot() +
  ...
  geom_sf(data = world)+
  geom_sf(data = Region, fill = NA, linewidth = .7)+
...
ggplot(data=ASC_CAP_MAB,aes(x=ASC_CAP,fill=condition))+
  geom_density(alpha=0.4)+
  geom_vline(xintercept=info_orig_MAB$ASC.CAP,color="darkorange",linetype="dashed",linewidth=1)+
  geom_vline(xintercept = 0.4,color="black",linetype="dashed",linewidth=1)+
  scale_fill_manual(values = c("green","red"))+
  labs(x="A/C",y="density")

ggplot(data=ASC_CAP_GOM,aes(x=ASC_CAP,fill=condition))+
  geom_density(alpha=0.4)+
  geom_vline(xintercept=info_orig_GOM$ASC.CAP,color=my.pal[4],linetype="dashed",linewidth=1)+
  geom_vline(xintercept = 0.4,color="black",linetype="dashed",linewidth=1)+
  labs(x="A/C",y="density")

ggplot(data=ASC_CAP,aes(x=ASC_CAP,fill=EPU))+
  geom_density(alpha=0.4)+
