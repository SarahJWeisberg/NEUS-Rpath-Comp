...
all_groups<-as.data.frame(cbind(group,model)) 
...
#3 Analyze and compare ecosense outputs Load ecosense outputs - bounded and unbounded
...
load(url("https://github.com/Nyelab/MAB-Rpath/blob/balance_prep/outputs/MAB_sense_Rpath_50k_2024.RData?raw=true"))
#without thermodynamic bounding
load(url("https://github.com/Nyelab/MAB-Rpath/blob/balance_prep/outputs/MAB_sense_unbound.RData?raw=true"))

#GB
#with thermodynamic bounding
load(url("https://github.com/NOAA-EDAB/GBRpath/blob/max_clam_edits/data/GB_sense_bound_bac.RData?raw=true"))
#without thermodynamic bounding
load(url("https://github.com/NOAA-EDAB/GBRpath/blob/max_clam_edits/data/GB_sense_unbound_bac.RData?raw=true"))
...
MAB_flow<- MAB_flow %>% mutate(EPU = "MAB") %>%
  ...
  mutate(condition = c(rep("bounded",length(MAB_sense)*(nliving+ndead)),rep("unbounded",length(MAB_sense_unbound)*(nliving+ndead))))
...
GB_flow<- GB_flow %>% mutate(EPU = "GB") %>% mutate(flow=as.numeric(flow)) %>% 
  mutate(condition = c(rep("bounded",length(GB_sense_bound_bac)*(nliving+ndead)),rep("unbounded",length(GB_sense_unbound_bac)*(nliving+ndead))))
...
##4.3 Comparing bounded/unbounded
...
for (j in 1:length(unique(flow$EPU))){
  ...
  for(i in 1:length(EPU_groups$group)){
    bound<-EPU_flow %>% filter(group == EPU_groups$group[i],condition == "bounded")
    unbound<-EPU_flow %>% filter(group == EPU_groups$group[i], condition == "unbounded")
...
#how does A/C compare between bounded and unbounded?
...
ASC_CAP_GOM<-as.data.frame(cbind(ASC_CAP_GOM,c(rep("bounded",length(GOM_sense)),rep("unbounded",length(GOM_sense_unbound)))))
...
bound<-ASC_CAP_GOM %>% filter(condition == "bounded") 
unbound<-ASC_CAP_GOM %>% filter(condition == "unbounded") 
...
ASC_CAP_GB<-as.data.frame(cbind(ASC_CAP_GB,c(rep("bounded",length(GB_sense_bound_bac)),rep("unbounded",length(GB_sense_unbound_bac)))))
...
#summary statistics
bound<-ASC_CAP_GB %>% filter(condition == "bounded") 
unbound<-ASC_CAP_GB %>% filter(condition == "unbounded") 
ks.test(bound$ASC_CAP,unbound$ASC_CAP)
bound %>% summarise(mean = mean(ASC_CAP), sd = sd(ASC_CAP))
unbound %>% summarise(mean = mean(ASC_CAP), sd = sd(ASC_CAP))
...
ASC_CAP_MAB<-as.data.frame(cbind(ASC_CAP_MAB,c(rep("bounded",length(MAB_sense)),rep("unbounded",length(MAB_sense_unbound)))))
...
bound<-ASC_CAP_MAB %>% filter(condition == "bounded")
unbound<-ASC_CAP_MAB %>% filter(condition == "unbounded") 
ks.test(bound$ASC_CAP,unbound$ASC_CAP)
bound %>% summarise(mean = mean(ASC_CAP), sd = sd(ASC_CAP))
unbound  %>% summarise(mean = mean(ASC_CAP), sd = sd(ASC_CAP))
...
bound<-ASC_CAP %>% filter(condition == "bounded")
model<-aov(data=bound,ASC_CAP~EPU)
TukeyHSD(model)
unbound<-ASC_CAP %>% filter(condition == "unbounded")
model<-aov(data=unbound,ASC_CAP~EPU)
TukeyHSD(model)
```
...
ggplot(aes(x=ASC_CAP, fill=condition))+
  ...
  geom_vline(xintercept=info_orig_GOM$ASC.CAP,color=my.pal2[4],linetype="dashed",linewidth=1)+
  geom_vline(xintercept = 0.4,color="black",linetype="dashed",linewidth=1)+
  scale_fill_manual(values = c(my.pal2[4],my.pal2[2]))+
...
Figure_3b<-ggplot(data=ASC_CAP_GB,aes(x=ASC_CAP,fill=condition))+
  geom_density(alpha=0.4)+
  geom_vline(xintercept=info_orig_GB$ASC.CAP,color=my.pal2[12],linetype="dashed",linewidth=1)+
  geom_vline(xintercept = 0.4,color="black",linetype="dashed",linewidth=1)+
  scale_fill_manual(values = c(my.pal2[12],my.pal2[10]))+
...
Figure_3c<-ggplot(data=ASC_CAP_MAB,aes(x=ASC_CAP,fill=condition))+
  geom_density(alpha=0.4)+
  geom_vline(xintercept=info_orig_MAB$ASC.CAP,color=my.pal2[8],linetype="dashed",linewidth=1)+
  geom_vline(xintercept = 0.4,color="black",linetype="dashed",linewidth=1)+
  scale_fill_manual(values = c(my.pal2[8],my.pal2[6]))+
...
Figure_3
```
##5.4 Figure 4: Flow metric comparisons (bounded to unbounded)
```{r}
games_test <- ASC_CAP %>% filter(condition == "bounded") %>% games_howell_test(ASC_CAP ~ EPU, detailed = T)

Figure_4b<-ASC_CAP %>% filter(condition == "bounded") %>%
  ...
  scale_color_manual(values=c(my.pal2[12],my.pal2[4],my.pal2[8]))+
  theme_Publication()+
  stat_pvalue_manual(games_test,label = "p.adj.signif",y.position = 0.5,step.increase = 0.05)+
  labs(y="A/DC",x="",title = "Constrained Condition")+
  theme(legend.position = "none",text = element_text(size=12))

games_test <- ASC_CAP %>% filter(condition == "unbounded") %>% games_howell_test(ASC_CAP ~ EPU, detailed = T)

Figure_4a<-ASC_CAP %>% filter(condition == "unbounded") %>%
  ggplot(aes(y=ASC_CAP,x=EPU,color=EPU))+
  geom_hline(yintercept = 0.4,color="black",linetype="dashed",linewidth=1)+
  geom_quasirandom(size = 1, width = .2, alpha = .25,)+
  stat_summary(fun = mean, geom = "point", shape = 95,size = 24) + 
  scale_color_manual(values=c(my.pal2[10],my.pal2[2],my.pal2[6]))+
  theme_Publication()+
  stat_pvalue_manual(games_test,label = "p.adj.signif",y.position = 0.5,step.increase = 0.05)+
  labs(y="A/DC",x="",title="Unconstrained Condition")+
  theme(legend.position = "none",text = element_text(size=12))
...
Figure_4
```
##5.6 Figures 6 and 7: Flow metric comparisons (bounded to unbounded)

```{r}

#define groups of interest
flow<-left_join(flow,ks_flow,by=c("group","EPU"))
my.pal3<-brewer.pal(11,"Spectral")

flow %>%
  filter(EPU == "GOM")%>%
  filter(condition == "unbounded") %>%
  ggplot(aes(x=flow,fill = ))+
  geom_density(alpha = 0.4)+
  #scale_fill_manual(values = c("green","red"))+
  #scale_fill_manual(values = c(my.pal2[8],my.pal2[10]))+
  #facet_wrap(facets = ~fct_reorder(group,p),nrow = 3,scales = "free")
...
Figure_7<-Figure_6d/Figure_6e/Figure_6f/Figure_6g
