...
GB_flow<- GB_flow %>% mutate(EPU = "GB") %>% mutate(flow=as.numeric(flow)) %>% 
  mutate(condition = c(rep("bounded",length(GB_sense)*(nliving+ndead)),rep("unbounded",length(GB_sense_unbound)*(nliving+ndead))))
...
ASC_CAP_GB<-as.data.frame(cbind(ASC_CAP_GB,c(rep("bounded",length(GB_sense)),rep("unbounded",length(GB_sense_unbound)))))
...
Figure_2
  #ggsave(filename= paste("figures/windows", name, ".jpg"), plot=p, width = 15, height=10, units=c("cm"), dpi=500)

```

##5.3 Figure 3: Flow metric comparisons (bounded to unbounded)

```{r}

#define groups of interest
flow<-left_join(flow,ks_flow,by="group")
ks_sig<-ks_flow %>% filter(p<0.05) %>% arrange(p)

flow %>% 
  #filter(group %in% ks_sig$group) %>%
  ...
  facet_wrap(vars(group),nrow=4,scales="free")
```

##5.4 Figure 4: A/C plots

```{r}
#set color palette
my.pal1<-brewer.pal(8,"Set2")
my.pal2<-brewer.pal(10,"Paired")
...
ggplot(data=ASC_CAP_GB,aes(x=ASC_CAP,fill=condition))+
  geom_density(alpha=0.4)+
  geom_vline(xintercept=info_orig_GB$ASC.CAP,color="blue",linetype="dashed",linewidth=1)+
  geom_vline(xintercept = 0.4,color="black",linetype="dashed",linewidth=1)+
  scale_fill_manual(values = c(my.pal2[8],my.pal2[10]))+
  labs(x="A/C",y="density")
...
ggplot(data=ASC_CAP_GOM,aes(x=ASC_CAP,fill=condition))+
  geom_density(alpha=0.4)+
  geom_vline(xintercept=info_orig_GOM$ASC.CAP,color=my.pal1[4],linetype="dashed",linewidth=1)+
  geom_vline(xintercept = 0.4,color="black",linetype="dashed",linewidth=1)+
  labs(x="A/C",y="density")
