...
```{r}
#GB
load(here("Rpath_models/alternate.GB.bal.rda"))
load(here("Rpath_models/alternate.GB.params.bal.rda"))
#rename
...
#GOM
load(here("Rpath_models/GOM_Rpath.RData"))
load(here("Rpath_models/GOM_params_Rpath.RData"))

#MAB
load(here("Rpath_models/MAB_Rpath_no_disc.RData"))
load(here("Rpath_models/MAB_params_Rpath_no_disc.RData"))
```
...
#2 Analyze and compare ecosense outputs Load ecosense outputs - nonselective and plausible

```{r}
#load ecosense outputs
#MAB
#with thermodynamic bounding
load(here("Ecosense_outputs/MAB_sense_Rpath_50k_2024.Rdata"))
#without thermodynamic bounding (nonselective condition)
load(here("Ecosense_outputs/MAB_sense_unbound.Rdata"))

#GB
#with thermodynamic bounding
load(here("Ecosense_outputs/GB_sense_bound.RData"))
#without thermodynamic bounding (nonselective condition)
load(here("Ecosense_outputs/GB_sense_unbound.RData"))

#GOM
#with thermodynamic bounding
load(here("Ecosense_outputs/GOM_sense_50k_resp_constrain.RData"))
#without thermodynamic bounding (nonselective condition)
load(here("Ecosense_outputs/GOM_sense_unbound.RData"))
```
...
#find index of pp
pp <- which(groups == "Phytoplankton")
#bind plausible and nonselective outputs
...
MAB_flow<- MAB_flow %>% mutate(EPU = "MAB") %>%
  ...
  mutate(condition = c(rep("plausible",length(MAB_sense)*(nliving+ndead)),rep("nonselective",length(MAB_sense_unbound)*(nliving+ndead))))
...
rm(alt_networks)
```
##3.2 Network conversion of GB ecosense outputs
...
#bind plausible and nonselective outputs
senses<-c(GB_sense,GB_sense_unbound)
...
GB_flow<- GB_flow %>% mutate(EPU = "GB") %>% mutate(flow=as.numeric(flow)) %>% 
  mutate(condition = c(rep("plausible",length(GB_sense)*(nliving+ndead)),rep("nonselective",length(GB_sense_unbound)*(nliving+ndead))))
...
GOM_flow<- GOM_flow %>% mutate(EPU = "GOM") %>% mutate(flow=as.numeric(flow)) %>% 
  mutate(condition = c(rep("plausible",length(GOM_sense)*(nliving+ndead)),rep("nonselective",length(GOM_sense_unbound)*(nliving+ndead))))
...
##3.3 Comparing nonselective/plausible
...
for (j in 1:length(unique(flow$EPU))){
  ...
  for(i in 1:length(EPU_groups$group)){
    bound<-EPU_flow %>% filter(group == EPU_groups$group[i],condition == "plausible")
    unbound<-EPU_flow %>% filter(group == EPU_groups$group[i], condition == "nonselective")
    test<-ks.test(bound$flow,unbound$flow)
    ks_flow<-rbind(ks_flow,c(EPU_groups$group[i],test$statistic,test$p.value,EPU_test))
  }
ks_flow<-as.data.frame(ks_flow)
colnames(ks_flow)<-c("group","D","p","EPU")
ks_flow$p<-as.numeric(ks_flow$p)
}


#how does A/C compare between plausible and nonselective?
...
ASC_CAP_GOM<-as.data.frame(cbind(ASC_CAP_GOM,c(rep("plausible",length(GOM_sense)),rep("nonselective",length(GOM_sense_unbound)))))
...
bound<-ASC_CAP_GOM %>% filter(condition == "plausible") 
unbound<-ASC_CAP_GOM %>% filter(condition == "nonselective") 
...
ASC_CAP_GB<-as.data.frame(cbind(ASC_CAP_GB,c(rep("plausible",length(GB_sense)),rep("nonselective",length(GB_sense_unbound)))))
...
#summary statistics
bound<-ASC_CAP_GB %>% filter(condition == "plausible") 
unbound<-ASC_CAP_GB %>% filter(condition == "nonselective") 
ks.test(bound$ASC_CAP,unbound$ASC_CAP)
bound %>% summarise(mean = mean(ASC_CAP), sd = sd(ASC_CAP))
#test for normality
ggqqplot(bound$ASC_CAP)
...
ASC_CAP_MAB<-as.data.frame(cbind(ASC_CAP_MAB,c(rep("plausible",length(MAB_sense)),rep("nonselective",length(MAB_sense_unbound)))))
...
bound<-ASC_CAP_MAB %>% filter(condition == "plausible")
unbound<-ASC_CAP_MAB %>% filter(condition == "nonselective") 
ks.test(bound$ASC_CAP,unbound$ASC_CAP)
bound %>% summarise(mean = mean(ASC_CAP), sd = sd(ASC_CAP))
unbound  %>% summarise(mean = mean(ASC_CAP), sd = sd(ASC_CAP))
#test for normality
...
bound<-ASC_CAP %>% filter(condition == "plausible")
test_bound<-kruskal_test(data=bound,ASC_CAP~EPU)
unbound<-ASC_CAP %>% filter(condition == "nonselective")
...
GB_web
#save
ggsave(filename = "GB_web.eps",GB_web,width=15,height = 10.8,dpi = 300)
...
Figure_3<-GOM_web+MAB_web +plot_layout(guides="collect")
Figure_3
...
games_test <- ASC_CAP %>% filter(condition == "plausible") %>% games_howell_test(ASC_CAP ~ EPU, detailed = T)

Figure_4<-ASC_CAP %>% filter(condition == "plausible") %>%
  ggplot(aes(y=ASC_CAP,x=EPU,color=EPU))+
  geom_hline(yintercept = 0.4,color="black",linetype="dashed",linewidth=0.75)+
  geom_hline(yintercept = 0.36,color="lightgrey",linetype="dashed",linewidth=0.75)+
  geom_hline(yintercept = 0.44,color="lightgrey",linetype="dashed",linewidth=0.75)+
  #geom_hline(yintercept = 0.44,color="grey",linetype="dashed",linewidth=1)+
  #geom_hline(yintercept = 0.36,color="grey",linetype="dashed",linewidth=1)+
  geom_quasirandom(size = 1, width = .2, alpha = .25)+
  stat_summary(fun = mean, geom = "point", shape =95,size = 40) + 
  ...
  labs(y="Relative Ascendency",x="",title = "Relative Ascendency Values from Ecosense Outputs")+
  theme(legend.position = "none",text = element_text(size=12))

Figure_4

ggsave(filename = "Fig4.jpg",Figure_4,width=9,height = 8.5,dpi = 300)
```

##4.4 Figure 5: Conceptual Figure
```{r}
#GOM
GOM_plaus <- ASC_CAP_GOM$ASC_CAP[which(ASC_CAP_GOM$condition == "plausible")]
GOM_density <- density(GOM_plaus, from = min(GOM_plaus), to = max(GOM_plaus)) %$% data.frame(x=x,y=y)
GOM_prob <- ecdf(GOM_plaus)
GOM_window <- GOM_prob(0.44) - GOM_prob(0.36)

ggplot(data = GOM_density, aes(x=x,y=y))+
  geom_line() +
  geom_area(data = subset(GOM_density, x >= 0.36 & x <= 0.44), fill = my.pal2[4], alpha = 0.8)+
  geom_area(data = subset(GOM_density, x <= 0.36), fill = my.pal2[5], alpha = 0.8)+
  geom_area(data = subset(GOM_density, x >= 0.44), fill = my.pal2[5], alpha = 0.8)+
  labs(x = "Relative Ascendency (A/DC)", y = "density", title = "GOM")+
  annotate(geom = "text", label = "P(in window) = 0.81", x = 0.45, y = 11)+
  theme_minimal(base_size=16, base_family="Arial")+
  xlim(0.30,0.475)+
  theme(axis.text.y = element_blank(),axis.ticks.y=element_blank())

#MAB
MAB_plaus <- ASC_CAP_MAB$ASC_CAP[which(ASC_CAP_MAB$condition == "plausible")]
MAB_density <- density(MAB_plaus, from = min(MAB_plaus), to = max(MAB_plaus)) %$% data.frame(x=x,y=y)
MAB_prob <- ecdf(MAB_plaus)
MAB_window <- MAB_prob(0.44) - MAB_prob(0.36)

ggplot(data = MAB_density, aes(x=x,y=y))+
  geom_line() +
  geom_area(data = subset(MAB_density, x >= 0.36 & x <= 0.44), fill = my.pal2[4], alpha = 0.8)+
  geom_area(data = subset(MAB_density, x <= 0.36), fill = my.pal2[5], alpha = 0.8)+
  geom_area(data = subset(MAB_density, x >= 0.44), fill = my.pal2[5], alpha = 0.8)+
  theme_minimal(base_size=16, base_family="Arial")+
  xlim(0.30,0.475)+
  labs(x = "Relative Ascendency (A/DC)", y = "density", title = "MAB")+
  annotate(geom = "text", label = "P(in window) = 0.84", x = 0.45, y = 13)+
  theme(axis.text.y = element_blank(),axis.ticks.y=element_blank())

#GB
GB_plaus <- ASC_CAP_GB$ASC_CAP[which(ASC_CAP_GB$condition == "plausible")]
GB_density <- density(GB_plaus, from = min(GB_plaus), to = max(GB_plaus)) %$% data.frame(x=x,y=y)
GB_prob <- ecdf(GB_plaus)
GB_window <- GB_prob(0.44) - GB_prob(0.36)

ggplot(data = GB_density, aes(x=x,y=y))+
  geom_line() +
  geom_area(data = subset(GB_density, x >= 0.36 & x <= 0.44), fill = my.pal2[4], alpha = 0.8)+
  geom_area(data = subset(GB_density, x <= 0.36), fill = my.pal2[5], alpha = 0.8)+
  geom_area(data = subset(GB_density, x >= 0.44), fill = my.pal2[5], alpha = 0.8)+
  theme_minimal(base_size=16, base_family="Arial")+
  xlim(0.30,0.475)+
  labs(x = "Relative Ascendency (A/DC)", y = "density", title = "GB")+
  annotate(geom = "text", label = "P(in window) = 0.73", x = 0.45, y = 11)
  theme(axis.text.y = element_blank(),axis.ticks.y=element_blank())
...
# Create data
x <- seq(0.3, 0.475, length.out = 1000)
df <- data.frame(x = x,y = dnorm(x, mean = mu, sd = sigma))
p_green <- pnorm(0.44, mu, sigma) - pnorm(0.36, mu, sigma)

ggplot(df, aes(x, y)) +
  geom_area(data = subset(df, x < 0.36), fill = my.pal2[5], alpha = 0.8) +
  geom_area(data = subset(df, x >= 0.36 & x <= 0.44),fill = my.pal2[4], alpha = 0.8) +
  geom_area(data = subset(df, x > 0.44), fill = my.pal2[5], alpha = 0.8) +
  labs(title = "GOM Simulation 1",x = "A/DC",y = "density") +
  geom_line()+
  xlim(0.30,0.475)+
  theme(axis.text.y = element_blank(), axis.ticks.y=element_blank())+
  theme_minimal(base_size=16, base_family="Arial")+
  annotate(geom = "text", label = "P(in window) = 0.79", x = 0.33, y = 11)+
  theme(axis.text.y = element_blank(),axis.ticks.y=element_blank())
...
ggplot(df, aes(x, y)) +
  ...
  geom_area(data = subset(df, x >= 0.36 & x <= 0.44),fill = my.pal2[4], alpha = 0.8) +
  geom_area(data = subset(df, x > 0.44), fill = my.pal2[5], alpha = 0.8) +
  labs(title = "GOM Simulation 2",x = "A/DC",y = "density") +
  geom_line()+
  xlim(0.30,0.475)+
  theme(axis.text.y = element_blank(), axis.ticks.y=element_blank())+
  theme_minimal(base_size=16, base_family="Arial")+
  annotate(geom = "text", label = "P(in window) = 0.68", x = 0.33, y = 7)+
  theme(axis.text.y = element_blank(),axis.ticks.y=element_blank())
...
ggplot(df, aes(x, y)) +
  ...
  geom_area(data = subset(df, x >= 0.36 & x <= 0.44),fill = my.pal2[4], alpha = 0.8) +
  geom_area(data = subset(df, x > 0.44), fill = my.pal2[5], alpha = 0.8) +
  labs(title = "GOM Simulation 3",x = "A/DC",y = "density") +
  geom_line()+
  xlim(0.30,0.475)+
  theme(axis.text.y = element_blank(), axis.ticks.y=element_blank())+
  theme_minimal(base_size=16, base_family="Arial")+
  annotate(geom = "text", label = "P(in window) = 0.93", x = 0.35, y = 11)+
  theme(axis.text.y = element_blank(),axis.ticks.y=element_blank())
```
...
Figure_S1a<-ASC_CAP_GOM %>%
  mutate(condition = fct_relevel(condition,"nonselective","plausible")) %>%
...
Figure_S1<-Figure_S1+plot_annotation(tag_levels = "A")
Figure_S1
ggsave(filename = "FigS1.jpg",Figure_S1,width=13.65,height = 10.8,dpi = 300)
...
ggsave(filename = "FigS2.jpg",Figure_S2,width=13.65,height = 10.8,dpi = 300)
ggsave(filename = "FigS3.jpg",Figure_S3,width=13.65,height = 10.8,dpi = 300)
ggsave(filename = "FigS4.jpg",Figure_S4,width=13.65,height = 10.8,dpi = 300)
